...
contract PRIVIPodERC721TokenRoyalty is Context, ERC721Burnable, NFTRoyalty {
     ...
     */
    constructor(string memory name, string memory symbol, string memory baseURI, address factory, uint256 royaltyAmount, address creator) 
        ERC721(name, symbol) 
        NFTRoyalty(royaltyAmount, creator) {
     ...
     */
    function marketSell(uint256 sellAmount, uint256 tokenId, address from, address to) public payable {
        // make sure all the fund is deposited here first
        require(msg.value == sellAmount, "PRIVIPodERC721TokenRoyalty: royalty amount is not correct");
