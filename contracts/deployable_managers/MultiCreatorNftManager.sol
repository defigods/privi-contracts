...
contract MultiCreatorNftManager {
    ...
    constructor(address[] memory _creators, uint256[] memory _shares) {
        require(_creators.length == _shares.length, "MultiCreatorNftManager: Creators and Shares are not of same length.");
        creators = _creators;
        shares = _shares;
    }
     ...
     */
    function getSharesTotal(uint256 _creatorIndex) public view returns(uint256 shareTotal){
        for (uint256 i = lastPaid[creators[_creatorIndex]]; i <= recieveCounter; i++) {
            shareTotal = shareTotal + ((shares[_creatorIndex] * recievedPayments[i]) / 100);
        }
    }
    ...
    function withdraw(uint256 _creatorIndex) external {
        require(msg.sender == creators[_creatorIndex], "MultiCreatorNftManager: You are not one of the creators.");
