...
contract("PRIVIFactory", (accounts) => {
    ...
    it("Mint 100 of podTokenId1 for investor 1", async () => {
        const priviPodTokenContract = await PRIVIPodToken.at(podAdress1);
        await priviFactoryContract.podMint(podId1, investor1, pod1TokenId1, 100, '0x1234');
        const investor1Balance = await priviPodTokenContract.balanceOf(investor1, pod1TokenId1);
        assert.deepEqual(investor1Balance.toString(), '100', "Error: investor 1 balance is not correct.");
    });

    it("Batch Mint 100 of podTokenId2, and 200 of podTokenId3 for investor 1", async () => {
        const priviPodTokenContract = await PRIVIPodToken.at(podAdress1);
        await priviFactoryContract.podMintBatch(podId1, investor1, [pod1TokenId2, pod1TokenId3], [100,200], '0x100200');
        const investor1BalancePod1TokenId2 = await priviPodTokenContract.balanceOf(investor1, pod1TokenId2);
        const investor1BalancePod1TokenId3 = await priviPodTokenContract.balanceOf(investor1, pod1TokenId3);
        assert.deepEqual(investor1BalancePod1TokenId2.toString(), '100', "Error: investor 1 balance of pod1TokenId2 is not correct.");
        assert.deepEqual(investor1BalancePod1TokenId3.toString(), '200', "Error: investor 1 balance of pod1TokenId3 is not correct.");
    });

    it("check balance of batch, 100 of podTokenId1 for investor3, and 100 of podTokenId2 for investor 4", async () => {
        const priviPodTokenContract = await PRIVIPodToken.at(podAdress1);
        await priviFactoryContract.podMint(podId1, investor3, pod1TokenId1, 100, '0x1001');
        await priviFactoryContract.podMint(podId1, investor4, pod1TokenId2, 100, '0x1002');
        const balanceArray = await priviPodTokenContract.balanceOfBatch([investor3, investor4], [pod1TokenId1, pod1TokenId2]);
        // console.log('balance Array: ',balanceArray);
        assert.deepEqual(balanceArray[0].toString(), '100', "Error: investor 3 balance of pod1TokenId1 is not correct.");
        assert.deepEqual(balanceArray[1].toString(), '100', "Error: investor 4 balance of pod1TokenId2 is not correct.");
    });
